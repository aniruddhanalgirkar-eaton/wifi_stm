...
static uint32_t*                     dma_data_source;
...
ISR_DIRECT_DECLARE(stm32_cyhal_dma2_irq_handler)
{
    ...
    result = cy_rtos_semaphore_set( &sdio_transfer_finished_semaphore );
    sem_count--;
    //WPRINT_WHD_DEBUG( ("J_semaphore = %d\n",result) );
...
static void sdio_prepare_data_transfer( cyhal_transfer_t direction, sdio_block_size_t block_size, /*@unique@*/ uint8_t* data, uint16_t data_size ) /*@modifies dma_data_source, user_data, user_data_size, dma_transfer_size@*/
{
    ...
    {
        dma_data_source = _temp_dma_buffer;
    }
    ...
    DMA2_Stream3->FCR  = (uint32_t) ( 0x00000021 | DMA_FIFOMODE_ENABLE | DMA_FIFO_THRESHOLD_FULL );
    		//FCR= 00-100-1-11 => 00-fifo status is empty-directmode disabled-full FIFO
    DMA2_Stream3->PAR  = (uint32_t) &SDIO->FIFO;
    //DMA2_Stream3->M0AR = (uint32_t) dma_data_source;
    DMA2_Stream3->M0AR = (uint32_t) p_dma_buffer;
    DMA2_Stream3->NDTR = dma_transfer_size/4; // transfer size

    WPRINT_WHD_DEBUG( ("J_NDTR=%d\n",DMA2_Stream3->NDTR) );
*/
...
cy_rslt_t cyhal_sdio_bulk_transfer(cyhal_sdio_t* obj, cyhal_transfer_t direction, uint32_t argument,
                                   const uint32_t* data, uint16_t length, uint32_t* response, sdio_transfer_mode_t mode)
{
    cy_rslt_t result = CYHAL_SDIO_RSLT_CANCELED;
   // CYHAL_ALIGN_DMA_BUFFER(static uint32_t  _temp_dma_buffer[_CYHAL_SDIO_DMA_BUFFER_SIZE]);
    DMA_HandleTypeDef hdma;

    WPRINT_WHD_INFO( ("J_cyhal_sdio_bulk_transfer\n") );
    ...
    if (obj->hsd->State == HAL_SD_STATE_READY)
    {
        ...
        WPRINT_WHD_DEBUG( ("J_length=%d,\targ.cmd53.count=%d\tBlock mode.block_size=%d,\tnumber_of_blocks=%d \n",length, arg.cmd53.count,block_size,number_of_blocks ) );


       //sdio_prepare_data_transfer( direction, block_size, (uint8_t*) data, length/*(block_size * number_of_blocks)*/ );
        //sdio_prepare_data_transfer( direction, block_size, (uint8_t*) data, arg.cmd53.count );

        data_size = block_size * number_of_blocks;
        ...
        //DMA2_Stream3->M0AR = (uint32_t) dma_data_source;
        DMA2_Stream3->M0AR = (uint32_t) p_dma_buffer;
        //DMA2_Stream3->NDTR = ((uint32_t) ( ( ( arg.cmd53.count + (uint16_t) block_size - 1 ) / (uint16_t) block_size ) * (uint16_t) block_size )) / 4;
 ...
 //ANN commented below for sometime. Below macro does ORing with SDMMC_CMD_CMDTRANS which is for CPSM Treating command as a Data Transfer
...
//ANN: disabling interrupt of buffer transfer complete
...
//ANN: below macro makes the CPSM not to treat command as a Data Transfer
...
//ANN: needs to flush the FIFO here...
	//obj->hsd->Instance->SDIO_FIFO   |= 0;
